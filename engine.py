...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for i in metric_logger.log_every(range(len(data_loader)), print_freq, header):            
        ...
        #     if k in weight_dict:
        #         print("k", k)
        #         print("loss_dict", loss_dict[k])
        #         print("weight_dict", weight_dict[k])
        ...
        metric_logger.update(orien_error=loss_dict_reduced['loss_orien'].item())
        metric_logger.update(trans_error=loss_dict_reduced['loss_trans'].item())
...
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, args):
    ...
    for step, (samples, targets) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        outputs = model(samples)
        loss_dict = criterion(outputs, targets, True) 
        weight_dict = criterion.weight_dict
