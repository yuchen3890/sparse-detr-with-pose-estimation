...
def cal_add_cuda(pred_RT, gt_RT, p3ds
    ...
    pred_p3ds = torch.mm(p3ds, pred_RT[:, :3].transpose(1, 0).float()) + pred_RT[:, 3]
...
def cal_adds_cuda(
    ...
    N, _ = p3ds.size()
    pd = torch.mm(p3ds, pred_RT[:, :3].transpose(1, 0).float()) + pred_RT[:, 3]
...
def eval_metric(
    ...
    pred_RT = pred_pose_lst
    gt_RT = RTs
    mesh_pts = get_pointxyz_cuda(cls_ids - 1).clone()
    add = cal_add_cuda(pred_RT, gt_RT, mesh_pts)
    adds = cal_adds_cuda(pred_RT, gt_RT, mesh_pts)
    cls_add_dis[cls_ids].append(add.item())
    cls_adds_dis[cls_ids].append(adds.item())
