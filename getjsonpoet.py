...
base_path = './ycbv_BOP/'
data_paths = ['train_pbr/', 'test/']
img_types = ['train', 'test']


output_base_path = './ycbv_BOP/annotations/'
annotation_paths = ['train.json', 'test.json']
    ...
    for img_dir in image_dirs:
        ...
        for img_name, b_k, p_k, c_k in zip(img_names, bbox_annotations, pose_annotations, camera_annotations):
            ...
            for bbox, pose, in zip(bbox_data, pose_data):
                ...
                T = [t / 1000.0 for t in pose['cam_t_m2c']]
                R = pose['cam_R_m2c']
                RT = []
                # # column first
                # for i in range(len(T)):
                #     RT.append([R[i], R[i+3], R[i+6], T[i]])
                
                # row first
                for i in range(0, len(R), 3):
                    RT.append([R[i], R[i+1], R[i+2], T[i // 3]])
